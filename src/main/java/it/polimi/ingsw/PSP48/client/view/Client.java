...
public class Client extends AbstractView implements Runnable, NetworkObserver
{
    ...
    public void requestMove(ArrayList<WorkerValidCells> validCellsForMove)
    {
        ...
        do {
            ...
            if (tempList.contains(chosenPositionToMove)) validMove=true; //we check if the list of positions contains the chosen position to move
            else
            {
    ...
    public void requestDomeOrBuild(ArrayList<WorkerValidCells> validForBuild, ArrayList<WorkerValidCells> validForDome)
    {
        ...
        for (WorkerValidCells cell : validForDome)
        {
            for (Position pos : cell.getValidPositions())
            {
                ...
                {
                    if (this.getCellOnBoard(pos.getRow(), pos.getColumn()).getCellColour() == ColoursForPrinting.red) this.getCellOnBoard(pos.getRow(), pos.getColumn()).setCellColour(ColoursForPrinting.green);
        ...
        if (chosenAction.equals("build"))
        {
            ...
            MoveCoordinates chosenCoordinates= new MoveCoordinates(workerRow, workerColumn, chosenRow, chosenColumn);
            this.notifyObserver(x->{ x.build(chosenCoordinates);});
        }
        else
        {
            ...
            MoveCoordinates chosenCoordinates= new MoveCoordinates(workerRow, workerColumn, chosenRow, chosenColumn);
            this.notifyObserver(x->{ x.dome(chosenCoordinates);});
        }
    }
