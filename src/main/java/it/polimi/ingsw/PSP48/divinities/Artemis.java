...
public class Artemis extends Divinity {
     ...
     */
    @Override
    public void move(int WorkerColumn, int WorkerRow, int moveColumn, int moveRow, Cell[][] gameCells, ArrayList<Divinity> divinitiesInGame) throws NotAdiacentCellException, IncorrectLevelException, OccupiedCellException, DomedCellException, DivinityPowerException {
        ...
        if (oldRowMove == -1 && oldColumnMove == -1) {
            ...
            if (oldRowMove == moveRow && oldColumnMove == moveColumn) throw new DivinityPowerException("Trying to return to original cell");
        }
